...
def run_full_pipeline(skip_streaming: bool = True):
    ...
    if overall_status != "FAILED":
        with pipeline_span("gold_analytics"):
            try:
                ...
                if gold_errors is None or "silver_load_failed" in (gold_errors or []):
                    status = "FAILED"
                    overall_status = "FAILED"
                    logger.error(f"Gold falhou criticamente: {gold_errors}")
                elif not gold_errors:
                    status = "SUCCESS"
                else:
                    status = "PARTIAL"
                    overall_status = "PARTIAL"
                ...
                run_logger.log_event(
                    "gold", "analytics",
                    status,
                    {"errors": gold_errors or ["null_return_error"]},
                )
            except Exception as exc:
    ...
    with pipeline_span("data_quality"):
        try:
            ...
            total_checks = quality_summary.get("total", 0)
            failed_checks = quality_summary.get("failed", 0)

            if total_checks == 0:
                qstatus = "FAILED_NO_CHECKS"
                logger.error("Quality check: Nenhum check foi executado!")
            ...
            logger.info(
                f"Quality: {quality_summary.get('passed', 0)}/{total_checks} "
                f"checks passaram ({quality_summary.get('pass_rate', 0)}%)"
            )
        except Exception as exc:

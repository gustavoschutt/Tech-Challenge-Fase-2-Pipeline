...
def sample_alunos() -> pd.DataFrame:
    return pd.DataFrame({
        "id_aluno": ["A01", "A02", "A03", "A04"],
        "id_municipio": ["3550308", "3550308", "3304557", "9999999"],
        "ano": [2023, 2023, 2023, 2023],
        "proficiencia": [750.0, 700.0, 743.0, 800.0],
        "_ingestion_timestamp": ["2024-01-01"] * 4,
        "_source": ["basedosdados"] * 4,
        "_batch_date": ["2024-01-01"] * 4,
    })
...
class TestTransformAlunos:
    def test_ponto_corte_743_alfabetizado(self, sample_alunos, sample_municipios, sample_ufs):
        ufs_silver = transform_ufs(sample_ufs)
        mun_silver = transform_municipios(sample_municipios, ufs_silver)
        result = transform_alunos(sample_alunos, mun_silver)
        
        # Aluno A01 (750.0 >= 743.0) -> True
        assert result[result["id_aluno"] == "A01"]["aluno_alfabetizado"].iloc[0] is True or result[result["id_aluno"] == "A01"]["aluno_alfabetizado"].iloc[0] == 1
        # Aluno A02 (700.0 < 743.0) -> False
        assert result[result["id_aluno"] == "A02"]["aluno_alfabetizado"].iloc[0] is False or result[result["id_aluno"] == "A02"]["aluno_alfabetizado"].iloc[0] == 0
        # Aluno A03 (743.0 >= 743.0) -> True
        assert result[result["id_aluno"] == "A03"]["aluno_alfabetizado"].iloc[0] is True or result[result["id_aluno"] == "A03"]["aluno_alfabetizado"].iloc[0] == 1

    def test_removes_invalid_municipio(self, sample_alunos, sample_municipios, sample_ufs):
        ufs_silver = transform_ufs(sample_ufs)
        mun_silver = transform_municipios(sample_municipios, ufs_silver)
        result = transform_alunos(sample_alunos, mun_silver)
        # Aluno A04 com id_municipio 9999999 deve ser descartado
        assert "A04" not in result["id_aluno"].values
        assert len(result) == 3

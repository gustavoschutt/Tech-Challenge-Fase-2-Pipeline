...
class DataQualityRunner:
    ...
    def summary(self) -> dict:
        ...
        return {
            ...
            "failed_checks": [f"{r.layer}/{r.entity}/{r.check_name}" for r in self.results if not r.passed],
            "status": "PASSED" if total > 0 and failed == 0 else ("FAILED" if failed > 0 else "NO_CHECKS_RUN"),
        }
    ...
    for layer, layer_entities in entities.items():
        for entity, config in layer_entities.items():
            ...
            try:
                ...
                logger.error(f"Arquivo não encontrado para {layer}/{entity}: {e}")
                runner.results.append(CheckResult(
                    check_name="arquivo_existente",
                    entity=entity,
                    layer=layer,
                    passed=False,
                    details=f"Arquivo Parquet não encontrado ou ilegível: {e}",
                ))
                continue

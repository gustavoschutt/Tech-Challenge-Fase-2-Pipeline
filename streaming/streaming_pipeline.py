...
class AlfabetizacaoEventProducer:
    ...
    def publisher(self):
        if self._publisher is None:
            try:
                ...
                logger.warning(f"Pub/Sub Publisher offline / sem credenciais GCP: {e}")
    ...
    def publish(self, event_type: str) -> str:
        ...
        if self.publisher:
            ...
            logger.info(f"Publicado no Pub/Sub [{event_type}] → message_id={message_id}")
            return message_id
        else:
            event_id = event["event_id"]
            logger.info(f"[SIMULAÇÃO LOCAL] Evento gerado [{event_type}] → ID: {event_id} | Payload: {event['payload']}")
            return event_id
...
class AlfabetizacaoEventConsumer:
    ...
    def subscriber(self):
        if self._subscriber is None:
            try:
                ...
                logger.warning(f"Pub/Sub Subscriber offline: {e}")
                self._subscriber = False
        return self._subscriber if self._subscriber is not False else None
    ...
    def bucket(self):
        if self._gcs is None:
            try:
                ...
                logger.warning(f"GCS client offline: {e}")
                self._gcs = False
        if self._gcs:
            return self._gcs.bucket(BUCKET_NAME)
        return None
    ...
    def _enrich_event(self, event: dict) -> dict:
        ...
        event["_processing_date"] = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    ...
    def _flush_buffer(self):
        ...
        blob_path = (
            f"bronze/streaming/date={datetime.now(timezone.utc).strftime('%Y-%m-%d')}/"
        ...
        if self.bucket:
            ...
            local_path = Path(f"/tmp/events_{timestamp}.json")
            local_path.write_text(content)
            logger.info(f"Flush local: {len(self._buffer)} eventos → {local_path}")
        self._buffer.clear()
        self._last_flush = time.time()

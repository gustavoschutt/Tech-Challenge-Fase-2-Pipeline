...
LEFT JOIN `silver_alfabetizacao.dim_municipios` mun
  ON ind.id_municipio = mun.id_municipio
LEFT JOIN `silver_alfabetizacao.dim_ufs` u
  ON mun.id_uf = u.id_uf
LEFT JOIN `silver_alfabetizacao.fct_meta_municipio` mm
  ON ind.id_municipio = mm.id_municipio AND ind.ano = mm.ano
LEFT JOIN `silver_alfabetizacao.fct_meta_brasil` mb
  ON ind.ano = mb.ano;
...
WITH agg_uf AS (
  SELECT
    ...
    SUM(ind.quantidade_matriculas) AS matriculas_total,
...
SELECT
  ...
  CURRENT_TIMESTAMP() AS _gold_timestamp
FROM agg_uf a
LEFT JOIN `silver_alfabetizacao.fct_meta_uf` muf
  ON a.id_uf = muf.id_uf AND a.ano = muf.ano;
...
SELECT
  ...
  COUNT(DISTINCT ind.id_municipio) AS total_municipios,
  COUNTIF(ind.meta_atingida = TRUE) AS municipios_meta_atingida,
  SUM(ind.quantidade_matriculas) AS total_matriculas,
  mb.meta_nacional,
